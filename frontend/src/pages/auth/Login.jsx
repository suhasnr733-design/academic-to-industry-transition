  ...
  return (
    <div className="w-full max-w-[440px] mx-auto bg-[#111827] backdrop-blur-xl p-6 sm:p-8 rounded-3xl shadow-2xl border border-gray-800/90 transition-all">
      ...
      <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex items-center justify-between text-xs pt-0.5">
          <label className="flex items-center text-gray-300 cursor-pointer select-none">
            <input
              ...
              type="checkbox"
              {...register('rememberMe')}
              className="h-3.5 w-3.5 text-indigo-600 focus:ring-indigo-500 border-gray-700 bg-[#1E293B] rounded cursor-pointer"
            />
            <span className="ml-2">Remember me</span>
          </label>

          <Link to="/forgot-password?role=student" className="text-indigo-400 hover:text-indigo-300 font-medium hover:underline">
            Forgot password?
          </Link>
        </div>
        ...
        <Button
          type="submit"
          className="w-full py-3 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-bold rounded-xl shadow-lg shadow-indigo-500/25 active:scale-[0.99] transition-all"
          isLoading={isLoading}
        >
          Sign In
        </Button>
